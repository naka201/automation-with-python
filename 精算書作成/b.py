...
def generate_settlement_files(input_file, output_files, file_cnt, specified_date=None):
    """
    ...
    for output_file in output_files:
        if "販売" in output_file:
            ...
            for purchaser, group in filtered_data.groupby('購入者'):
                ...
                purchaser_file = os.path.join(folder_path, f"{invo}{invo_num}_{purchaser}.xlsx")
            ...
            for seller, group in filtered_data.groupby('出品者'):
                ...
                # ファイルをコピーして出品者名に変更
                invo_num = "{:03d}".format(file_cnt)
                seller_file = os.path.join(folder_path, f"{invo}{invo_num}_{sellers_dict[seller]}.xlsx")
    ...
    print(f"{error_list}の対応する出品者が見つかりませんでした。")
...
generate_settlement_files(input_file, [settlement_sell, settlement_buy], file_cnt, specified_date)