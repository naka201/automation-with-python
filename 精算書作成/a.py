...
def generate_settlement_files(input_file, output_file, file_cnt, specified_date=None):
    ...
    if "販売" in output_file:
        ...
        for purchaser, group in filtered_data.groupby('購入者'):
            ...
            # ファイルをコピーして購入者名に変更
            purchaser_file = os.path.join(folder_path, f"{file_cnt}_{purchaser}.xlsx")
        ...
        for seller, group in filtered_data.groupby('出品者'):
            ...
            # ファイルをコピーして出品者名に変更
            seller_file = os.path.join(folder_path, f"{file_cnt}_{seller}.xlsx")
...
generate_settlement_files(input_file, settlement_buy, file_cnt)
generate_settlement_files(input_file, settlement_sell, file_cnt)